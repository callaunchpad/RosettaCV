...
from trainer.meta_trainer import maml_outer_train_loop, maml_inner_train, maml_update_parameters
...
def train_maml(args):
    ...
    maml_outer_train_loop(model, optimizer, train_tasks, validation_tasks, args.batch_size, args.num_iters_inner,
                     args.num_iters_outer, maml_inner_train, maml_update_parameters, device, args.save_name)
...
if __name__ == "__main__":
    ...
    ap.add_argument('--save_name', default='maml-test.p')
    args = ap.parse_args()

    wandb.init('RosettaCV')
    wandb.config.update(args)
    wandb.config.model = "resnet18"
    wandb.config.type = "MAML"
    train_maml(args)
