...
def train_mpl(teacher_model,
    ...
    if not t_optimizer:
        ...
        t_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(t_optimizer, num_epochs*(num_iter//batch_size))
    if not s_optimizer:
        s_optimizer = torch.optim.Adam(student_model.parameters(), lr=learning_rate, weight_decay=1e-5)
        s_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(s_optimizer, num_epochs*(num_iter//batch_size))
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(num_iter):
            ...
            if global_step % 100 == 0:
                ...
                wandb.log({ 'batch_teacher_loss': t_loss.item(),
